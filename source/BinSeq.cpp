...
bool BinSeq::loadRecord(uint irec, char** buff, std::array<uint, 3> lens)
{
    ...
    binseq_context_copy_primary(context, buff[0], lenR1); // copy primary sequence to buffer
    lens[0] += len;

    if (isPE) {
        len = binseq_record_decode_extended(record, context); // decode auxiliary sequence
        binseq_context_copy_extended(context, buff[1], lenR2); // copy auxiliary sequence to buffer
        lens[1] += len;
    };
